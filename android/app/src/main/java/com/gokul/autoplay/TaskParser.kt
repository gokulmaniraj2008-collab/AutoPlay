...
object TaskParser {
    ...
    fun parse(command: String, nowMillis: Long = System.currentTimeMillis()): ParseResult {
        ...
        val action = when {
            ...
            isYouTubeSearchCommand(lower) -> PhoneTask.Action.YOUTUBE_SEARCH
        ...
        if (action == PhoneTask.Action.UNKNOWN) {
            return ParseResult(null, "Try: open YouTube, search Tamil songs, turn on the flashlight, or use a supported phone command.")
        }
        ...
        val target = when (action) {
            ...
            PhoneTask.Action.YOUTUBE_SEARCH -> parseYouTubeSearch(raw)
    ...
    private fun isFlashlightOnCommand(lower: String): Boolean {
        ...
        val on = "turn on" in lower || "switch on" in lower || "light on" in lower || lower.endsWith(" on") || lower == "light"
    ...
    private fun isYouTubeSearchCommand(lower: String): Boolean {
        val search = "search" in lower || "find" in lower || "look for" in lower || "play" in lower
        val youtube = "youtube" in lower || "you tube" in lower
        val songLike = "song" in lower || "music" in lower || "video" in lower
        return search && (youtube || songLike)
    }
    ...
    private fun parseYouTubeSearch(raw: String): String {
        return raw
            .replace(Regex("(?i)\\b(open|launch)\\s+youtube\\b"), "")
            .replace(Regex("(?i)\\b(?:search|find|look for)\\b"), "")
            .replace(Regex("(?i)\\b(?:on|in)\\s+(?:youtube|you tube)\\b"), "")
            .replace(Regex("(?i)\\b(?:youtube|you tube)\\b"), "")
            .replace(Regex("(?i)\\b(?:play)\\b"), "")
    ...
    private fun parseWhatsAppTarget(raw: String): String {
        ...
        if (match != null) return "${match.groupValues[1].trim()}|||${match.groupValues[2].trim()}"
        return "|||"
    }
